...
export const questionsSlice = createSlice({
  ...
  reducers: {
    toggleLike: (state, action: PayloadAction<QuestionID>) => {
      ...
      if (isLiked) {
        state.likedQuestionsIds = state.likedQuestionsIds.filter(
...
export const createQuestion = createAppAsyncThunk<
  QuestionID,
  ICreateQuestionForm
>("questions/createQuestion", async (data, { dispatch }) => {
...
export const loadLikedQuestionIds = createAppAsyncThunk(
  ...
  async (_, { dispatch }) => {
    const loaded: QuestionID[] = JSON.parse(
      localStorage.getItem(LIKED_QUESTIONS_KEY) || "[]",
    );
